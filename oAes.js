(function (thiz) {
	...
	// adapted for O by Francesco Sullo
		...
		SetKey: function (password,nBits) {
			if (O.isArray(password) && password.length == nBits/8) key = password;
			else {
...
//		  var nonce = (new Date()).getTime();  // milliseconds since 1-Jan-1970
		  ...
		  var nonce = parseInt(
				  	(new Date()).getTime().toString()   // milliseconds since 1-Jan-1970
				  	+ Math.random().toString().substring(2,6) // a random salt
				  ,10);
	...
	O.expand({
        ...
        fromAES: function (str,key,nbits) {
                return str && key ? O.__.AES.AESDecryptCtr(
                        str.toString(),
                        key,
                        nbits ? nbits : 256
                ) : '';
        ...
        toAES: function (str,key,nbits) {
               return str && key ? O.__.AES.AESEncryptCtr(
                        str.toString(),
                        key,
                        nbits ? nbits : 256
               ) : '';
